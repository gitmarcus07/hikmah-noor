...
export async function onRequestPut(context) {
  ...
  for (const k of Object.keys(body)) {
    ...
    let v = body[k];
    if (['second_reminder', 'alerts_on', 'onboarded', 'hide_hasanat'].includes(k)) v = v ? 1 : 0;
  ...
  const runUpdate = (s, v) => env.DB.prepare(`UPDATE prefs SET ${s.join(', ')} WHERE user_id = ?`).bind(...v).run();
  try {
    await runUpdate(sets, vals);
  } catch (e) {
    // Pre-0007 DB (missing hide_hasanat): persist the rest.
    if (!/no such column/i.test(String((e && e.message) || ''))) throw e;
    const keepS = [], keepV = [];
    for (let i = 0; i < sets.length; i++) {
      if (/^hide_hasanat = \?$/.test(sets[i])) continue;
      keepS.push(sets[i]);
      keepV.push(vals[i]);
    }
    keepV.push(user.id); // WHERE user_id = ?
    if (!keepS.length || keepS.length === 1) return json({ error: 'nothing_to_update' }, 400);
    await runUpdate(keepS, keepV);
  }
  const prefs = (await row(env, user.id)) || {};
