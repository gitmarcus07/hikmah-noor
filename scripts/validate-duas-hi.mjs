// Hindi dua overlay validator (shape-only): no unknown/duplicate slugs,
// required fields per entry, Devanagari+Latin only.
// Run: npm run content:duas:hi
...
console.log(`\n📊 Duas: ${mainSlugs.length} — Hindi overlay: ${ovSlugs.length} (${Math.round((ovSlugs.length / mainSlugs.length) * 100)}%)`);
if (errors) { console.error(`\n${errors} error(s). Fix before building.`); process.exit(1); }
console.log('\n✅ Hindi dua overlays valid.');
