// Hindi guide overlay validator (shape-only): no unknown/duplicate slugs,
// required fields per entry, steps where EN has steps, Devanagari+Latin only.
// Run: npm run content:guides:hi
...
let txt = '';
for (const f of ['guides-hi-1.ts', 'guides-hi-2.ts', 'guides-hi-3.ts', 'guides-hi-4.ts', 'guides-hi-5.ts']) {
...
// Hindi overlays: Devanagari + Latin + digits/punct only (no Arabic/Urdu script, no CJK/Cyrillic)
const bad = txt.match(/[\u0600-\u06FF\u0400-\u04FF\u3040-\u30FF\u4E00-\u9FFF\uFF00-\uFFEF]/);
if (bad) fail(`suspect non-Hindi character U+${bad[0].codePointAt(0).toString(16)}`);

console.log(`OK guides-hi: ${mainSlugs.length} guides, ${ovSlugs.length} Hindi overlays (${Math.round((ovSlugs.length / mainSlugs.length) * 100)}%)`);
if (errors) { console.error(`\n${errors} error(s). Fix before building.`); process.exit(1); }
console.log('All Hindi guide overlays valid.');
